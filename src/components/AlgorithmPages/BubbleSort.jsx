...
export default function BubbleSort({ lang = 'tr' }) {
  ...
  const barColor = (i) => {
    ...
    if (step.swapped.includes(i)) return 'from-rose-400 to-rose-600';
  ...
  const pseudocode = `procedure bubbleSort(A)
  n = length(A)
  for i = 0 to n-2
    for j = 0 to n-2-i
      if A[j] > A[j+1]
    ...
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-6 animate-fade-in">
      ...
      <div className="card">
        ...
        <div className="flex flex-wrap justify-center gap-4 mt-4 pt-4 border-t border-gray-100 dark:border-gray-800 text-xs">
          {[
            { color: 'bg-indigo-500', label: t(lang, 'legend.normal') },
            { color: 'bg-amber-500',  label: t(lang, 'legend.comparing') },
            { color: 'bg-rose-500',   label: t(lang, 'legend.swapping') },
            { color: 'bg-emerald-500',label: t(lang, 'legend.sorted') },
          ].map(l => (
